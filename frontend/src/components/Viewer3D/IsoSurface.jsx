...
export default function IsoSurface() {
  ...
  const object = useMemo(() => {
    ...
    const latCount = slices[0].values.length; const lonCount = slices[0].values[0].length
    ...
    const min = Number.isFinite(colorMin) ? colorMin : Math.min(...finite); const max = Number.isFinite(colorMax) ? colorMax : Math.max(...finite)
    ...
    const sample = (x, y, z) => {
      const xi = Math.min(lonCount - 1, Math.round((x / (RESOLUTION - 1)) * (lonCount - 1))); const yi = Math.min(latCount - 1, Math.round((y / (RESOLUTION - 1)) * (latCount - 1))); const zi = Math.min(slices.length - 1, Math.round((z / (RESOLUTION - 1)) * (slices.length - 1)))
      const value = Number(slices[zi]?.values?.[yi]?.[xi]); return Number.isFinite(value) ? value : target
    }
    const scale = max - min
    ...
    mc.update()
    const topY = depthToY(depths[0], verticalExaggeration); const bottomY = depthToY(depths[depths.length - 1], verticalExaggeration)
    mc.position.set(0, (topY + bottomY) / 2, 0); mc.scale.set(SCENE_WIDTH / 2, Math.max(1, Math.abs(bottomY - topY) / 2), SCENE_DEPTH / 2); mc.frustumCulled = false
    return mc
