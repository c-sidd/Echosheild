...
// Keep the browser render budget bounded. The active level is always retained.
...
export function selectRenderDepths(depths, activeDepth, maxDepths = MAX_RENDER_DEPTHS) {
  ...
  for (let i = 0; i < maxDepths; i += 1) {
    selected.add(unique[Math.round((i * (unique.length - 1)) / (maxDepths - 1))])
  }
  ...
  while (values.length > maxDepths) {
    ...
    for (let i = 1; i < values.length - 1; i += 1) {
      const isActive = Number.isFinite(activeDepth) && values[i] === activeDepth
...
export function useDatasets() {
  return useQuery({ queryKey: ['datasets'], queryFn: ({ signal }) => model.fetchDatasets(signal), staleTime: MIN })
}

export function useMetadata(id) {
  return useQuery({ queryKey: ['metadata', id], queryFn: ({ signal }) => model.fetchMetadata(id, signal), enabled: !!id, staleTime: LONG })
}

export function useVariables(id) {
  return useQuery({ queryKey: ['variables', id], queryFn: ({ signal }) => model.fetchVariables(id, signal), enabled: !!id, staleTime: LONG })
}

export function useTimes(id) {
  return useQuery({ queryKey: ['times', id], queryFn: ({ signal }) => model.fetchTimes(id, signal), enabled: !!id, staleTime: LONG })
}

export function useDepths(id) {
  return useQuery({ queryKey: ['depths', id], queryFn: ({ signal }) => model.fetchDepths(id, signal), enabled: !!id, staleTime: LONG })
}

export function useExtent(id) {
  return useQuery({ queryKey: ['extent', id], queryFn: ({ signal }) => model.fetchExtent(id, signal), enabled: !!id, staleTime: LONG })
}

export function useTimesList(id) {
  return useQuery({ queryKey: ['times-list', id], queryFn: ({ signal }) => model.fetchTimesList(id, signal), enabled: !!id, staleTime: LONG })
}
...
export function useSlice(id, variable, timeIndex, depth, bbox) {
  return useQuery({
    ...
    queryFn: ({ signal }) => model.fetchSlice(id, variable, timeIndex, depth, bbox, signal),
...
export async function fetchSliceStack(queryClient, id, variable, timeIndex, activeDepth = null, signal) {
  const depths = await queryClient.fetchQuery({
    queryKey: ['depths', id],
    queryFn: ({ signal: depthSignal }) => model.fetchDepths(id, depthSignal),
    staleTime: LONG,
  })
...
export function prefetchSlices(queryClient, id, variable, timeIndexes, depth, bbox) {
  ...
  for (const t of timeIndexes) {
    ...
    void queryClient.prefetchQuery({
      ...
      queryFn: ({ signal }) => model.fetchSlice(id, variable, t, depth, bbox, signal),
      staleTime: SLICE_STALE,
    })
  }
}
...
export function useProfile(id, variable, lat, lon, timeIndex) {
  return useQuery({
    ...
    queryFn: ({ signal }) => model.fetchProfile(id, variable, lat, lon, timeIndex, signal),
    enabled: !!id && !!variable && Number.isFinite(lat) && Number.isFinite(lon),
    staleTime: SLICE_STALE,
  })
}
...
export function usePoint(id, variables, lat, lon, timeIndex, depth, enabled = true) {
  return useQuery({
    ...
    queryFn: ({ signal }) => model.fetchPoint(id, variables, lat, lon, timeIndex, depth, signal),
    enabled: enabled && !!id && Array.isArray(variables) && variables.length > 0 && Number.isFinite(lat) && Number.isFinite(lon),
    staleTime: SLICE_STALE,
  })
}
...
export function useServices(id) {
  return useQuery({ queryKey: ['services', id], queryFn: ({ signal }) => model.fetchServices(id, signal), enabled: !!id, staleTime: LONG, retry: false })
}
...
export function useArgoDetail(wmo) {
  return useQuery({ queryKey: ['argo-detail', wmo], queryFn: ({ signal }) => argo.fetchArgoDetail(wmo, signal), enabled: wmo != null, staleTime: MIN, retry: false })
}

export function useArgoProfile(wmo) {
  return useQuery({ queryKey: ['argo-profile', wmo], queryFn: ({ signal }) => argo.fetchArgoProfile(wmo, undefined, signal), enabled: wmo != null, staleTime: MIN, retry: false })
}

export function useGliderStatus() {
  return useQuery({ queryKey: ['glider-status'], queryFn: ({ signal }) => fetchGliderStatus(signal), staleTime: LONG, retry: false })
}

export function useHealth() {
  return useQuery({ queryKey: ['health'], queryFn: ({ signal }) => fetchHealth('/health', signal), refetchInterval: 60_000, staleTime: 30_000, retry: false })
}

export function useReadiness() {
  return useQuery({ queryKey: ['health-ready'], queryFn: ({ signal }) => fetchHealth('/health/ready', signal), refetchInterval: 60_000, staleTime: 30_000, retry: false })
}
...
export function useDatasetSync() {
  ...
  useEffect(() => {
    ...
    if (Array.isArray(ext.depth_levels) && ext.depth_levels.length) store.setDepths(ext.depth_levels)
    ...
    if (!current.length && Array.isArray(ext.variables) && ext.variables.length) {
      store.setVariables(ext.variables.map((v) => (typeof v === 'string' ? { name: v, canonical_name: v } : v)))
    }
  }, [extentQuery.data])

  useEffect(() => {
    if (Array.isArray(variablesQuery.data) && variablesQuery.data.length) useOceanStore.getState().setVariables(variablesQuery.data)
  }, [variablesQuery.data])

  useEffect(() => {
    if (Array.isArray(timesListQuery.data)) useOceanStore.getState().setTimestampsList(timesListQuery.data)
  }, [timesListQuery.data])
}
