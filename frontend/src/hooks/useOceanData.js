...
import { fetchGliderStatus, fetchGliderMissions, fetchGliderProfiles } from '@/services/gliderService'
...
export function useSlice(id, variable, timeIndex, depth, bbox) {
  return useQuery({ queryKey: sliceKey(id, variable, timeIndex, depth, bbox), queryFn: ({ signal }) => model.fetchSlice(id, variable, timeIndex, depth, bbox, signal), enabled: !!id && !!variable && Number.isFinite(depth), staleTime: SLICE_STALE, placeholderData: (prev) => prev })
}
export const sliceStackKey = (id, variable, timeIndex) => ['slice-stack', id, variable, timeIndex]
export async function fetchSliceStack(queryClient, id, variable, timeIndex) {
  const depths = await queryClient.fetchQuery({ queryKey: ['depths', id], queryFn: ({ signal }) => model.fetchDepths(id, signal), staleTime: LONG })
  if (!Array.isArray(depths) || !depths.length) return []
  return Promise.all(depths.map((depth) => queryClient.fetchQuery({ queryKey: sliceKey(id, variable, timeIndex, depth, null), queryFn: ({ signal }) => model.fetchSlice(id, variable, timeIndex, depth, null, signal), staleTime: SLICE_STALE })))
}
export function useSliceStack(id, variable, timeIndex) { return useQuery({ queryKey: sliceStackKey(id, variable, timeIndex), queryFn: () => fetchSliceStackInternal(id, variable, timeIndex), enabled: !!id && !!variable, staleTime: SLICE_STALE }) }
async function fetchSliceStackInternal(id, variable, timeIndex) { const depths = await model.fetchDepths(id); if (!Array.isArray(depths) || !depths.length) return []; return Promise.all(depths.map((depth) => model.fetchSlice(id, variable, timeIndex, depth, null))) }
export function prefetchSliceStack(queryClient, id, variable, timeIndex) { if (!id || !variable || !Number.isFinite(timeIndex)) return; void queryClient.prefetchQuery({ queryKey: sliceStackKey(id, variable, timeIndex), queryFn: () => fetchSliceStack(queryClient, id, variable, timeIndex), staleTime: SLICE_STALE }) }
export function prefetchSlices(queryClient, id, variable, timeIndexes, depth, bbox) { if (!id || !variable || !Number.isFinite(depth)) return; for (const t of timeIndexes) { if (!Number.isFinite(t)) continue; void queryClient.prefetchQuery({ queryKey: sliceKey(id, variable, t, depth, bbox), queryFn: ({ signal }) => model.fetchSlice(id, variable, t, depth, bbox, signal), staleTime: SLICE_STALE }) } }
...
export function useGliderMissions(enabled = true) { return useQuery({ queryKey: ['glider-missions'], queryFn: ({ signal }) => fetchGliderMissions(signal), enabled, staleTime: MIN, refetchInterval: MIN, retry: false }) }
export function useGliderProfiles(missionId) { return useQuery({ queryKey: ['glider-profiles', missionId], queryFn: ({ signal }) => fetchGliderProfiles(missionId, signal), enabled: !!missionId, staleTime: MIN, retry: false }) }
...
export function useDatasetSync() {
  ...
  useMetadata(id); const times = useTimes(id); const depths = useDepths(id); const variables = useVariables(id)
  useEffect(() => { if (times.data?.start && times.data?.end != null) useOceanStore.getState().setTimeRange(times.data) }, [times.data])
  useEffect(() => { if (Array.isArray(depths.data) && depths.data.length) useOceanStore.getState().setDepths(depths.data) }, [depths.data])
  useEffect(() => { if (Array.isArray(variables.data)) useOceanStore.getState().setVariables(variables.data) }, [variables.data])
}
